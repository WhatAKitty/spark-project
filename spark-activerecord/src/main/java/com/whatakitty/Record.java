...
public class Record implements Serializable {
	...
	/**
	 * Get values from record by prefix.
	 * @param prefix
	 * @return
	 */
	public Object[] getColumnValuesByPrefix(String prefix) {
	    List<Object> result = new ArrayList<Object>();
	    Set<String> attrNameSet = getColumns().keySet();
	    Iterator<String> attrNameIter = attrNameSet.iterator();
	    
	    while(attrNameIter.hasNext()) {
	        String attrName = attrNameIter.next();
	        if(attrName.startsWith(prefix)) {
	            result.add(get(attrName));
	        }
	    }
	...
	public String toJson() {
		return JsonKit.toJson(getColumns(), 4);
	}
}
